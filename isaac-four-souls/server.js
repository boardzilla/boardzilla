...
game.setupBoard = board => {
  ...
  dungeon.addComponent('counter', {display: 'hp', initialValue:1, max: 8, left: 220, top: 100});
...
game.hideBoard(`card[flipped], #player-mat:not(.mine) #hand card, #loot card, #treasure card, #monsters card, #characters card, #eternals card`, ['front', 'text', 'edition']);
...
const startingActions = ['start', 'intoCharDeckTop', 'intoEternalDeckTop'];
Object.entries(editions).forEach(([i, edition]) => {
  startingActions.push(`excludeEdition-${i}`);
  game.actions[`excludeEdition-${i}`] = {
...
game.play = async () => {
  ...
  let action;
  game.prompt('Select starting characters and eternals, then hit Start');
  do {
  ...
  [1,2,3].forEach(() => {
    const soul = bonusSouls.splice(game.random(bonusSouls.length), 1)[0];
    addCard(soul, game.board.find('#bonus-souls'));
  });

  const allActions = Object.keys(game.actions).filter(a => !startingActions.includes(a));
  while(true) {
